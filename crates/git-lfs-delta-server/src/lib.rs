...
use git_lfs_delta_core::{ChunkDescriptor, ChunkStream, ObjectManifest, ObjectOid};
use git_lfs_delta_protocol::{
...
/// Shared dependencies for the Git LFS Delta HTTP service.
#[derive(Clone)]
pub struct AppState {
    pool: PgPool,
    chunks: ChunkStore,
...
async fn metrics(State(state): State<AppState>) -> String {
    format!(
        "# TYPE git_lfs_delta_logical_upload_bytes_total counter\n\
git_lfs_delta_logical_upload_bytes_total {}\n\
# TYPE git_lfs_delta_logical_download_bytes_total counter\n\
git_lfs_delta_logical_download_bytes_total {}\n\
# TYPE git_lfs_delta_received_chunk_bytes_total counter\n\
git_lfs_delta_received_chunk_bytes_total {}\n\
# TYPE git_lfs_delta_forgejo_auth_cache_hits_total counter\n\
git_lfs_delta_forgejo_auth_cache_hits_total {}\n\
# TYPE git_lfs_delta_forgejo_auth_cache_misses_total counter\n\
git_lfs_delta_forgejo_auth_cache_misses_total {}\n\
# TYPE git_lfs_delta_forgejo_requests_total counter\n\
git_lfs_delta_forgejo_requests_total {}\n\
# TYPE git_lfs_delta_http_requests_total counter\n\
git_lfs_delta_http_requests_total {}\n\
# TYPE git_lfs_delta_http_failures_total counter\n\
git_lfs_delta_http_failures_total {}\n\
# TYPE git_lfs_delta_http_in_flight gauge\n\
git_lfs_delta_http_in_flight {}\n",
...
async fn upload_basic(
    ...
    if headers
        ...
        .is_some_and(|length| length > git_lfs_delta_core::MAX_OBJECT_SIZE)
    {
        return Err(ApiError::new(
    ...
    let mut limited = reader.take(git_lfs_delta_core::MAX_OBJECT_SIZE + 1);
    let copied = tokio::io::copy(&mut limited, &mut file)
        .await
        .map_err(ApiError::io)?;
    if copied > git_lfs_delta_core::MAX_OBJECT_SIZE {
        return Err(ApiError::new(
            StatusCode::PAYLOAD_TOO_LARGE,
            "object exceeds the 100 GiB production limit",
        ));
    ...
    let manifest = tokio::task::spawn_blocking(move || {
        ChunkStream::new(source, git_lfs_delta_core::ChunkingProfile::beta_v1())
            .finish()
    ...
    let producer = tokio::task::spawn_blocking(move || {
        let mut stream = ChunkStream::new(source, git_lfs_delta_core::ChunkingProfile::beta_v1());
    ...
    for descriptor in &manifest.chunks {
        let bytes = state
            ...
            .map_err(|error| match error {
                git_lfs_delta_storage::StorageError::Provider(object_store::Error::NotFound {
                    ..
...
fn action_url(
    ...
    owner: &str,
    repository: &str,
    oid: git_lfs_delta_core::ObjectOid,
...
impl ApiError {
    ...
    fn storage(error: git_lfs_delta_storage::StorageError) -> Self {
        if matches!(
            error,
            git_lfs_delta_storage::StorageError::DigestMismatch { .. }
        ) {
