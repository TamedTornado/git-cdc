...
async fn oidc_validates_signature_issuer_audience_and_database_grant() {
    ...
    let state = AppState::new_oidc(
        ...
        Url::parse("http://git-lfs-delta.example/").unwrap(),
    ...
    let exp = SystemTime::now()
        ...
        .as_secs()
        + 300;
    let valid = signed_token("alice", &issuer, "git-lfs-delta", exp, b"super-secret");
    assert_eq!(
        app.clone()
    ...
    let reader = signed_token("bob", &issuer, "git-lfs-delta", exp, b"super-secret");
    assert_eq!(
        app.clone()
    ...
    for invalid in [
        ...
        signed_token("alice", &issuer, "git-lfs-delta", exp, b"wrong-secret"),
    ] {
        assert_eq!(
            app.clone()
