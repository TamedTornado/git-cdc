...
async fn setup_with_store() -> (PgPool, axum::Router, Arc<InMemory>) {
    ...
    sqlx::query("DROP TRIGGER IF EXISTS git_lfs_delta_test_fail_publish ON object_chunks")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("DROP FUNCTION IF EXISTS git_lfs_delta_test_fail_publish()")
        .execute(&pool)
        .await
        .unwrap();
...
async fn database_failure_rolls_back_logical_object_publication() {
    ...
    sqlx::query(
        "CREATE FUNCTION git_lfs_delta_test_fail_publish() RETURNS trigger \
    ...
    .unwrap();
    sqlx::query(
        "CREATE TRIGGER git_lfs_delta_test_fail_publish BEFORE INSERT ON object_chunks \
         FOR EACH ROW EXECUTE FUNCTION git_lfs_delta_test_fail_publish()",
    )
    .execute(&pool)
    .await
    .unwrap();
