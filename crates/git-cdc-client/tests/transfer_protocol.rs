...
use git_cdc::{BackendError, DownloadResult, TransferBackend, run_transfer_protocol};

#[derive(Default)]
struct FakeBackend;

impl TransferBackend for FakeBackend {
    fn upload(&self, _request: &git_cdc::UploadRequest) -> Result<u64, BackendError> {
        Ok(12)
    }

    fn download(
        &self,
        _request: &git_cdc::DownloadRequest,
    ) -> Result<DownloadResult, BackendError> {
        Ok(DownloadResult {
            path: "/tmp/git-cdc-download".into(),
