...
use git_lfs_delta_core::ObjectManifest;
use git_lfs_delta_protocol::{BatchRequest, BeginUploadRequest, BeginUploadResponse, Operation};
...
fn initialize_repository(repository: &std::path::Path, cache: &std::path::Path, base: &str) {
    ...
    run(
        repository,
        env!("CARGO_BIN_EXE_git-lfs-delta"),
        cache,
        &["install", "--scope", "local"],
    );
    run(
