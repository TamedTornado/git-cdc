...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bind: SocketAddr = std::env::var("GIT_CDC_BIND")
        ...
        .parse()?;
    let maximum_connections = optional_usize("GIT_CDC_DATABASE_MAX_CONNECTIONS", 20)?;
    let maximum_basic_transfers = optional_usize("GIT_CDC_MAX_BASIC_TRANSFERS", 2)?;
    let maximum_data_requests = optional_usize("GIT_CDC_MAX_DATA_REQUESTS", 64)?;
    let staging_root = PathBuf::from(
        std::env::var("GIT_CDC_STAGING_DIR")
            .unwrap_or_else(|_| std::env::temp_dir().to_string_lossy().into_owned()),
    );
    if auth_mode == "development"
        && !bind.ip().is_loopback()
        && std::env::var_os("GIT_CDC_ALLOW_REMOTE_DEVELOPMENT_AUTH").is_none()
    {
        return Err("development authentication may not bind remotely without GIT_CDC_ALLOW_REMOTE_DEVELOPMENT_AUTH".into());
    }
