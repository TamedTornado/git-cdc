...
docker compose -f "$root/docker-compose.test.yml" exec -T postgres \
  psql -v ON_ERROR_STOP=1 -U git_cdc git_cdc \
  -c 'DROP SCHEMA public CASCADE; CREATE SCHEMA public;'

export GIT_CDC_DATABASE_URL=postgres://git_cdc:git_cdc@127.0.0.1:55433/git_cdc
export GIT_CDC_BASE_URL=http://127.0.0.1:58081/
export GIT_CDC_STORAGE_URL="file://$work/live-storage"
export GIT_CDC_AUTH_MODE=development
export GIT_CDC_DEV_TOKEN=backup-secret
export GIT_CDC_BIND=127.0.0.1:58081
"$root/target/debug/git-cdc-admin" repository-add team assets >/dev/null
start_server
...
git -C "$repository" config user.email backup@git-cdc.invalid
git -C "$repository" config http.extraheader 'Authorization: Bearer backup-secret'
git -C "$repository" lfs install --local
(cd "$repository" && "$root/target/debug/git-cdc" install --scope local)
(cd "$repository" && "$root/target/debug/git-cdc" configure --scope local --url http://127.0.0.1:58081/team/assets/info/lfs)
...
docker compose -f "$root/docker-compose.test.yml" exec -T postgres \
  pg_dump -U git_cdc --clean --if-exists --no-owner git_cdc >"$work/database.sql"
...
docker compose -f "$root/docker-compose.test.yml" exec -T postgres \
  psql -v ON_ERROR_STOP=1 -U git_cdc git_cdc <"$work/database.sql"
start_server
...
(cd "$repository" && "$root/target/debug/git-cdc" uninstall --scope local)
