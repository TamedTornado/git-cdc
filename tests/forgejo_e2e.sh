...
export GIT_LFS_DELTA_BASE_URL=http://127.0.0.1:58080/
export GIT_LFS_DELTA_STORAGE_URL="file://$work/storage"
...
git -C "$source_repo" config user.email integration@git-lfs-delta.invalid
git -C "$source_repo" remote add origin http://127.0.0.1:53000/alice/assets.git
git -C "$source_repo" lfs install --local
(cd "$source_repo" && "$root/target/debug/git-lfs-delta" install --scope local)
(cd "$source_repo" && "$root/target/debug/git-lfs-delta" configure --scope local --url http://127.0.0.1:58080/alice/assets/info/lfs)
(cd "$source_repo" && "$root/target/debug/git-lfs-delta" doctor)
(cd "$source_repo" && "$root/target/debug/git-lfs-delta" status) | grep -Fq 'lfs.customtransfer.cdc.args=transfer'
(cd "$source_repo" && "$root/target/debug/git-lfs-delta" status) | grep -Fq 'lfs.url=http://127.0.0.1:58080/alice/assets/info/lfs'
git -C "$source_repo" lfs track '*.bin'
asset_bytes="${GIT_LFS_DELTA_ACCEPTANCE_ASSET_BYTES:-268435456}"
...
logical_before="$(metric git_lfs_delta_logical_upload_bytes_total)"
physical_before="$(metric git_lfs_delta_received_chunk_bytes_total)"
printf 'localized Git LFS Delta edit' | dd of="$source_repo/asset.bin" bs=1 seek=$((asset_bytes / 2)) conv=notrunc status=none
git -C "$source_repo" add asset.bin
git -C "$source_repo" commit -m 'Localized asset edit'
git -C "$source_repo" push
logical_after="$(metric git_lfs_delta_logical_upload_bytes_total)"
physical_after="$(metric git_lfs_delta_received_chunk_bytes_total)"
...
(cd "$clone" && "$root/target/debug/git-lfs-delta" install --scope local)
(cd "$clone" && "$root/target/debug/git-lfs-delta" configure --scope local --url http://127.0.0.1:58080/alice/assets/info/lfs)
git -C "$clone" lfs pull
git -C "$clone" lfs fsck --objects
cmp "$source_repo/asset.bin" "$clone/asset.bin"
(cd "$clone" && "$root/target/debug/git-lfs-delta" uninstall --scope local)
